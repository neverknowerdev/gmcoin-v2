...
# GMCoin v2 Database Migration Runner
...
apply_migration() {
    ...
    local version=$2
    local full_name=$3  # Full filename without .sql (e.g., "001_initial_schema.up")
    local hash=$4
...
rollback_migration() {
    local version=$1
    local full_name=$2  # Full filename without .sql (e.g., "001_initial_schema.up")
...
run_up() {
    ...
    echo "🚀 Starting GMCoin v2 database migrations (UP)..."
...
run_down() {
    ...
    echo "🔄 Starting GMCoin v2 database migrations (DOWN)..."
    echo "Database URL: ${DB_CONNECTION_STRING}"
    echo "Migrations directory: ${MIGRATIONS_DIR}"
    echo ""
